...
2. 对章节计算字符 2/3-gram TF-IDF 余弦分数：rule_raw 正文60%、法规依据正文15%、标题25%；取 top-k；每个长章节截取与两项查询重叠最高的窗口后，
   交给本地 Qwen 选择证据。

`rule_text` 中的公式、开发说明、其他生成块及 `structured_fields` 不参与第一阶段召回。Qwen 返回空选择时保留已达到
`minimum_score` 的字符候选作为兜底；这只表示保留候选供步骤三判断，不表示候选必然违规。
...
`rule_raw` 决定审查主题，法规依据补充具体要求，`check_method` 仅用于路由。字段语义映射时，模型收到的是将
