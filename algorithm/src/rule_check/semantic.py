...
2. 只有 rule_raw 或 legal_basis_reference 明确要求的外部文件、数值或比较对象未在证据中提供时，才能返回 insufficient_input；不得根据公式、开发说明或预设字段推断缺失输入。
3. 证据只是“未提及、未记载、未明确说明”某项行为、程序或结果时，不得据此推定 violation 或 warning。只有当证据出现与规则要求明确抵触的表述、或明确缺少法定必备内容时，才可判 violation。原文对某程序作出合规描述而未逐一复述执行细节的，视为符合，不因“没写清楚是否照做”而判违规。
4. warning 仅用于证据已显示明确瑕疵或风险、但尚不足以认定为确定违规的情形，且同样必须回引至少一条冲突原文；不得用 warning 表达“信息不全所以存疑”。
5. 证据不足以判断是否合规、又不满足第 2 条的 insufficient_input 条件时，应判 pass，并在 analysis 说明“依据现有证据未发现与规则抵触之处”，而不是默认 warning。
6. 没有明确命中时不得仅凭关键词判违规，必须结合上下文和规则条件。
7. status=pass 时 findings 应为空；violation/warning 时至少提供一条可回引证据。
8. 不得把 evidence 数组位置臆测为某类外部文件，只能使用 location 中明确提供的信息。"""
...
def _to_result(rule: MatchedRule, data: dict) -> RuleResult:
    ...
    for item in data.get("findings") or []:
        try:
            ...
            continue  # 证据下标缺失或非法：丢弃该 finding，不再整条清零
        if evidence_index < 0 or evidence_index >= len(rule.evidence):
            continue  # 越界：同样丢弃该 finding
        quote = str(item.get("quote", "")).strip()
        reason = str(item.get("reason", "")).strip()
        source_quote = _source_quote(rule.evidence[evidence_index].text, quote)
        if source_quote is not None:
            findings.append(Finding(evidence_index=evidence_index, quote=source_quote, reason=reason))
        elif quote:
            # 引用无法逐字回映射：保留判定与模型原始引用，但计入未校验，避免一个格式差异把整条规则清零成 error。
            findings.append(Finding(evidence_index=evidence_index, quote=quote, reason=reason))
            unverified += 1
...
def evaluate_semantic(rule: MatchedRule) -> SemanticEvaluation:
    ...
    for attempt in range(1, settings.semantic_max_retries + 2):
        ...
        try:
            # 合规判定要可复现，且 quote 必须逐字回引原文，故用贪婪解码（temperature=0）保证稳定与忠实。
